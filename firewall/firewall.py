...
def handle_packet(packet):
    if TCP in packet and packet[TCP].flags == "S":
        ...
        if scan_tracker[src_ip]["count"] > 5:
            print(f"IP {src_ip} exceeded the limit of 5 SYN packets, blocking for {BLOCK_DURATION}")
            block_ip(src_ip)
        ...
        syn_ack = (
            ...
            TCP(dport=src_port, sport=port, flags="SA", seq=100, ack=packet[TCP].seq + 1)
        )
        ...
        data_packet = (
            IP(dst=src_ip, src=packet[IP].dst) /
            TCP(dport=src_port, sport=port, flags="PA", seq=101, ack=packet[TCP].seq + 1) /
            Raw(load="Try harder")
        )
